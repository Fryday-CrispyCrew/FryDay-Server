...
|`THIS`
|이번 인스턴스만 수정합니다. 해당 인스턴스에 override 값이 저장되며, 반복 규칙은 변경되지 않습니다.

|`THIS_AND_FUTURE`
|이번 인스턴스 날짜부터 이후의 모든 인스턴스에 적용합니다. 기존 반복 규칙은 해당 날짜 하루 전에 종료되고, 새로운 반복 규칙이 생성됩니다.

|`ALL`
|반복 규칙 전체에 적용합니다. 개별적으로 override된 인스턴스는 override 값이 유지됩니다.
|===
...
|`THIS`
|이번 인스턴스만 삭제합니다. 반복 규칙은 유지됩니다.

|`THIS_AND_FUTURE`
|이번 인스턴스 날짜부터 이후의 모든 인스턴스를 삭제하고, 반복 규칙을 해당 날짜 하루 전에 종료합니다.

|`ALL`
|반복 규칙 전체와 모든 인스턴스를 삭제합니다.
|===
